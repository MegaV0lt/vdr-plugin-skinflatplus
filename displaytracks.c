...
cFlatDisplayTracks::cFlatDisplayTracks(const char *Title, int NumTracks, const char * const *Tracks) {
    ...
    img_ac3 = ImgLoader.GetIcon("tracks_ac3", kIconMaxSize, m_FontHeight);
    img_stereo = ImgLoader.GetIcon("tracks_stereo", kIconMaxSize, m_FontHeight);
    ...
    m_CurrentIndex = -1;
    const int TitleWidth {m_Font->Width(Title) + FontCache.GetStringWidth(m_FontName, m_FontHeight, " ")};
...
void cFlatDisplayTracks::SetItem(const char *Text, int Index, bool Current) {
    ...
    cString TextWithSpace = cString::sprintf(" %s", Text);
    if (Index == -1)
