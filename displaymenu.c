...
bool cFlatDisplayMenu::SetItemEvent(const cEvent *Event, int Index, bool Current, bool Selectable,
    ...
    if (Event && Selectable) {
        ...
        try {
            ...
            std::string sep = Event->Title();  // TODO: Use std:string_view
            if (sep.size() > 12) {
...
void cFlatDisplayMenu::SetText(const char *Text, bool FixedFont) {
    ...
    bool Scrollable = ComplexContent.Scrollable(Height - m_MarginItem2);
    if (!Scrollable) {
...
void cFlatDisplayMenu::DrawMainMenuWidgets(void) {
    ...
    int wHeight = MenuPixmap->ViewPort().Height() - m_MarginItem2;
    int ContentTop {0};
