...
concat(
  ...
  concat(
    ...
    -- Beschreibung von Extern oder DVB
    case when sub_longdescription is Null and cnt_longdescription is Null then '|| >>> Keine Beschreibung verfügbar! <<<' else
      case when sub_longdescription is Null then concat('||', cnt_longdescription, ' [DVB]') else
    ...
    -- Staffel und Episode aus Kurztext, falls vorhanden
    ...
    case when epi_part is Null then
      case when sub_shorttext is Null then '' else
        case when REGEXP_INSTR(sub_shorttext, 'E[0-9]+.') = 0 then '' else
          concat('|Episode: ', TRIM(LEADING '0' FROM TRIM(LEADING 'E' FROM REGEXP_SUBSTR(sub_shorttext, 'E[0-9]+')))) end end
    else
